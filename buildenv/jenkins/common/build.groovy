...
def fetchFile(src, dest) {
    ...
    // move a dataset to the current directory
    sh "mv '//${fileToMove}' ${src}"
    // append the dataset to core.xxx.dmp file
    sh "cat ${src} >> ${dest}"
    // remove the dataset from the current directory
    sh "rm ${src}"
...
def archive_diagnostics() {
    if (SPEC.contains('zos')) {
        ...
        for (match in matches) {
            ...
            def numFiles = num.trim()
            numFiles = numFiles.toInteger()
            numFiles ++
        ...
        sh "find . -name 'core.*.dmp' -o -name 'javacore.*.txt' -o -name 'Snap.*.trc' -o -name 'jitdump.*.dmp'" 
        sh "ls"
        // Note: to preserve the files ACLs set _OS390_USTAR=Y env variable (see variable files)
